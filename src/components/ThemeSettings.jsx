...
const ThemeSettings = () => {
  ...
  const { currentColor, setColor, currentMode, setMode, setActiveThemeSettings } = useStateContext();

  const blockClass = "ml-2 text-2xl text-white block";
  const hiddenClass = "ml-2 text-2xl text-white hidden";
    ...
    <div className="bg-half-transparent w-screen fixed nav-item top-0 right-0">
      <div className="float-right h-screen dark:text-gray-200 bg-white dark:bg-[#484B52] w-400">
        <div className="flex justify-between items-center p-4 ml-4">
          ...
          <button type="button" onClick={() => setActiveThemeSettings(false)} style={{ color: "rgb(153, 171, 180)", borderRadius: "50%"}} className="text-2xl p-3 hover:drop-shadow-xl hover:bg-light-gray">
            <MdOutlineCancel />
          </button>
        </div>
              ...
              <TooltipComponent key={index} content={color.name} position="TopCenter">
                <div className="relative mt-2 cursor-pointer flex gap-5 items-center">
                  <button type="button" className="h-10 w-10 rounded-full cursor-pointer" style={{ backgroundColor: color.color }} onClick={() => setColor(color.color)}>
                    <BsCheck className={currentColor === color.color ? blockClass : hiddenClass}/>
                  </button>
                </div>
              </TooltipComponent>
            ))}
